...
function isSharedRecord(record: TLRecord) {
  return (
...
export function bindTldrawToYjs({
  ...
  function scheduleFlush() {
    // Drawing can produce a record for every pointer event. A short batch
    // keeps the reliable data channel responsive for captions and chat too,
    // without making a stroke feel delayed to another participant.
    if (!timer) timer = setTimeout(flush, 48);
  }

  function queue(changes: TldrawChanges) {
    for (const record of Object.values(changes.added)) {
      if (isSharedRecord(record)) pending.set(record.id, record);
    }
    for (const [, record] of Object.values(changes.updated)) {
      if (isSharedRecord(record)) pending.set(record.id, record);
    }
    for (const record of Object.values(changes.removed)) {
      if (isSharedRecord(record)) pending.set(record.id, null);
    }
  ...
  function applyRecords(ids: Iterable<string>) {
    ...
    for (const id of ids) {
      ...
      // Session records (camera, selection, focus mode) belong to one browser.
      // A prior board build persisted some of them beside the real document;
      // never merge those records back into an editor.
      if (record && isSharedRecord(record)) {
        additions.push(structuredClone(record));
      } else if (
        !record &&
        /^(document|page|shape|asset|binding):/.test(id) &&
        store.has(id as TLRecord["id"])
      ) {
